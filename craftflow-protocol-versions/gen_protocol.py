...
def prepare_dir(version, dir_mod_name, state):
    path = f"src/v{version}/"
    if not os.path.exists(path):
        os.makedirs(path)
        with open("src/lib.rs", "a") as f:
            f.write(f"pub mod v{version};\n")
...
def generate_protocol_direction(version: int, protocol, prev_version: Optional[int], prev_protocol, c2s: bool):
    ...
    for packet in packets:
        ...
        if os.path.exists(f"src/V{version}/{dir_mod_name}/{state}/{packet}.rs"):
            continue
        ...
        if spec == prev_spec:
            ...
            generated_rust = f"pub use crate::v{prev_version}::{dir_mod_name}::{state}::{snake_to_pascal(packet)};"
        else:
            generated_rust = gen_packet(spec)

        prepare_dir(version, dir_mod_name, state)

        with open(f"src/v{version}/{dir_mod_name}/{state}/mod.rs", "a") as f:
            f.write(f"pub mod {packet};\n")

        with open(f"src/v{version}/{dir_mod_name}/{state}/{packet}.rs", "w") as f:
