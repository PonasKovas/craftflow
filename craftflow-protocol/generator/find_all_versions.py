...
def find_all_versions() -> Dict[str, Path]:
    ...
    with open(repo_path / "data" / "pc" / "common" / "protocolVersions.json", "r") as f:
        non_classic_versions = [item["minecraftVersion"] for item in json.loads(f.read())]
    ...
    for version_dir in (repo_path / "data" / "pc").iterdir():
        ...
        if not ((version_dir / "version.json").is_file() and (version_dir / "protocol.json").is_file()):
            print(Fore.YELLOW + Style.NORMAL + "Skipping " + Fore.MAGENTA + str(version_dir) + Fore.YELLOW + " (no version.json or protocol.json)")
            continue
        ...
        if version_data["minecraftVersion"] not in non_classic_versions:
            print(Fore.YELLOW + Style.NORMAL + "Skipping " + Fore.MAGENTA + str(version_dir) + Fore.YELLOW + " (not in common versions list)")
            continue
        ...
        if not all(char.isdigit() or char == '.' for char in version_data["minecraftVersion"]) or \
            ...
            print(Fore.YELLOW + Style.NORMAL + "Skipping " + Fore.MAGENTA + str(version_dir) + Fore.YELLOW + " (not release)")
            continue
