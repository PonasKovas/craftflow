...
pub fn gen_impl_code(item_name: &Ident, data: &Data, is_cow: bool) -> TokenStream {
	match data {
		Data::Struct(data) => {
...
fn gen_cow(item_name: &Ident, data: &DataEnum) -> TokenStream {
	...
	let owned = data
		.variants
		.iter()
		.find(|variant| Attributes::parse_variant(&variant.attrs).owned)
		.expect("one variant must be marked as owned");
	let borrowed = data
		.variants
		.iter()
		.find(|variant| Attributes::parse_variant(&variant.attrs).borrowed)
