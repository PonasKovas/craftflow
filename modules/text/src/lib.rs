...
impl<'a> Text<'a> {
	pub fn serialize(&self) -> String {
		serde_json::to_string(self).expect("this cant fail bro")
	}
	pub fn deserialize(input: &'a str) -> Result<Self, serde_json::Error> {
...
macro_rules! text {
    ($text:expr $(, $key:ident $(= $value:expr)? )* ) => {
        $crate::Text::Object(::std::boxed::Box::new(
        	$crate::TextObject {
	            content: $crate::TextContent::Text {
					text: $text.into()
				},
	            extra: ::std::default::Default::default(),
	            $($key: text!(@format $key $(= $value)?),)*
	            ..<$crate::TextObject as ::std::default::Default>::default()
         	}
        ))
    };
