...
impl NbtSeq<NbtString> {
	// todo generalise it to any dynamic structure using an epic trait and move to nbtseq.rs
	pub fn new(iter: impl IntoIterator<Item = impl AsRef<str>>) -> Self {
		...
		Self::new_raw(Bytes::from(bytes), len)
	}
}

impl NbtFormat for NbtSeq<NbtString> {
