...
pub fn gen_random_dyn_nbt(tags_n: usize) -> NbtValue {
    ...
    while i < tags_n {
        ...
        match Tag::new(rng.random_range(1..12)).unwrap() {
            ...
            Tag::String => {
                current_parent.insert(k, NbtValue::String(gen_random_string(&mut rng, 2usize.pow(16)-1)));
            }
            Tag::ByteArray => {
                let length = rng.random_range(0..1024*1024);
                current_parent.insert(k, NbtValue::ByteArray(NbtByteArray((&mut rng).random_iter().take(length).collect())));
            },
            Tag::IntArray => {
                let length = rng.random_range(0..512*1024);
                current_parent.insert(k, NbtValue::IntArray(NbtIntArray((&mut rng).random_iter().take(length).collect())));
            },
            Tag::LongArray => {
                let length = rng.random_range(0..1024);
                current_parent.insert(k, NbtValue::LongArray(NbtLongArray((&mut rng).random_iter().take(length).collect())));
            },
            Tag::List => {
                let length = rng.random_range(0..1024*1024);
                match Tag::new(rng.random_range(1..12)).unwrap() {
                    Tag::Byte => {
                        current_parent.insert(k, NbtValue::List(NbtList::Byte((&mut rng).random_iter().take(length).collect())));
                    },
                    Tag::Short => {
                        current_parent.insert(k, NbtValue::List(NbtList::Short((&mut rng).random_iter().take(length/2).collect())));
                    },
                    Tag::Int => {
                        current_parent.insert(k, NbtValue::List(NbtList::Int((&mut rng).random_iter().take(length/4).collect())));
                    },
                    Tag::Long => {
                        current_parent.insert(k, NbtValue::List(NbtList::Long((&mut rng).random_iter().take(length/8).collect())));
                    },
                    Tag::Float => {
                        current_parent.insert(k, NbtValue::List(NbtList::Float((&mut rng).random_iter().take(length/4).collect())));
                    },
                    Tag::Double => {
                        current_parent.insert(k, NbtValue::List(NbtList::Double((&mut rng).random_iter().take(length/8).collect())));
                    },
                    Tag::String => {
                        current_parent.insert(k, NbtValue::List(NbtList::String(iter::from_fn(|| Some(gen_random_string(&mut rng, 512))).take(length).collect())));
                    },
